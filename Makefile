...
stop:
	docker stop gurosh-emission-node
	docker rm gurosh-emission-node
...
build:
	...
	docker build -t gurosh/emission/node  .
	docker run --env-file ./.env --restart=always  -it -p 5050:5050 --name gurosh-emission-node gurosh/emission/node
